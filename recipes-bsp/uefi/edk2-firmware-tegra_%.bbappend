...
# Set EDGEOS_EFI_DEBUG = "1" in local.conf or distro config to enable verbose UEFI debug output
...
# Override EDK2 build mode based on EDGEOS_EFI_DEBUG variable
# Default: "0" (RELEASE mode - production, minimal output)
# Set to "1" for DEBUG mode (verbose UEFI debug output)
EDK2_BUILD_RELEASE = "${@'0' if d.getVar('EDGEOS_EFI_DEBUG') == '1' else '1'}"
