#!/bin/bash
#
# EdgeOS Device Name Generation Script
...
ADJECTIVES_FILE="/usr/share/edgeos/adjectives.txt"
NOUNS_FILE="/usr/share/edgeos/nouns.txt"

# Logging
log() {
    echo "[$(date '+%Y-%m-%d %H:%M:%S')] $*"
    logger -t edgeos-device-name "$*" || true
}
...
generate_random_name() {
    ...
    if [ ! -f "$ADJECTIVES_FILE" ] || [ ! -f "$NOUNS_FILE" ]; then
        ...
        local uuid_file="/etc/edgeos/device-uuid"
        if [ -f "$uuid_file" ]; then
...
main() {
    log "Starting EdgeOS device name generation"
    ...
    logger -t edgeos-device-name "Generated device name: $new_name"
}
