...
SRC_URI += " \
    ${@'file://journald-persistent.conf file://var-log.mount' if d.getVar('EDGEOS_PERSIST_JOURNAL_LOGS') == '1' else ''} \
    "

# Enable var-log.mount unit when journal persistence is enabled
SYSTEMD_SERVICE:${PN} += "${@'var-log.mount' if d.getVar('EDGEOS_PERSIST_JOURNAL_LOGS') == '1' else ''}"
SYSTEMD_AUTO_ENABLE:${PN} = "enable"

do_install:append() {
    if [ "${EDGEOS_PERSIST_JOURNAL_LOGS}" = "1" ]; then
        ...
        install -D -m0644 ${WORKDIR}/journald-persistent.conf ${D}${systemd_unitdir}/journald.conf.d/10-edgeos-persistent.conf
